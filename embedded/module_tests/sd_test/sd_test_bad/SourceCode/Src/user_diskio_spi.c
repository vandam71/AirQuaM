...
//If you want to use a different SPI port change this handle accordingly
...
#define FCLK_SLOW() { SPI_HANDLE.Instance->I2SPR = 256; }	/* Set SCLK = slow */
#define FCLK_FAST() { SPI_HANDLE.Instance->I2SPR = 16; }	/* Set SCLK = fast */

//#define USE_CS_SPI		//uncomment if more than one device is being used on the spi bus
#ifdef USE_CS_SPI
...
{
	BYTE rxDat;
    HAL_SPI_TransmitReceive(&SPI_HANDLE, &dat, &rxDat, 1, 50);
    return rxDat;
}
